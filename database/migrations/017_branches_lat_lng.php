<?php
/**
 * Al-Riaz Associates — Migration 017: Add `lat` / `lng` to `branches`
 *
 * Lets the admin pin each office location precisely so the public-site
 * "Get Directions" button can route to exact coordinates instead of a
 * free-text address (which Google sometimes geocodes incorrectly).
 *
 * Idempotent.
 *
 * Run:
 *   - Browser (localhost): http://localhost/al-riaz/database/migrations/017_branches_lat_lng.php
 *   - SSH (server):        php /path/to/site/database/migrations/017_branches_lat_lng.php
 */
...
try {
    ...
    foreach (['lat', 'lng'] as $col) {
        $row = $db->query("SHOW COLUMNS FROM `branches` LIKE '$col'")->fetch();
        if ($row) {
            $log[] = ['ok', "Column `$col` already present on `branches`."];
        } else {
            $after = $col === 'lat' ? 'address' : 'lat';
            $db->exec("ALTER TABLE `branches` ADD COLUMN `$col` DECIMAL(10,7) DEFAULT NULL AFTER `$after`");
            $log[] = ['ok', "Added `$col` column to `branches`."];
        }
    }

    $log[] = ['done', 'Migration 017 complete.'];
} catch (Throwable $e) {
...
<head>
<meta charset="UTF-8">
<title>Migration 017 — branches lat/lng</title>
...
<body>
  <div class="card p-4">
    <h3 class="mb-3">Migration 017 — <code>branches.lat</code> / <code>branches.lng</code></h3>
    ...
    <p class="mt-3 mb-0 fs-13 text-muted">
      Next: open <code>/admin/settings.php?tab=agency</code>, paste latitude / longitude for each office, then save.
    </p>
  </div>
</body>
</html>
