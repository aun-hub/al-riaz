<?php
/**
 * Al-Riaz Associates — Migration 015: properties.listing_type / possession_status → VARCHAR
 *
 * Converts the two ENUM columns to VARCHAR(40) so admins can add custom
 * options (Listing Type, Possession Status) from /admin/settings.php
 * without a schema change every time.
 *
 * Idempotent — checks current column type before altering.
 *
 * Run:
 *   - Browser (localhost): http://localhost/al-riaz/database/migrations/015_properties_taxonomy_varchar.php
 *   - SSH (server):        php /path/to/site/database/migrations/015_properties_taxonomy_varchar.php
 */
...
try {
    ...
    $cols = [
        'listing_type'      => "VARCHAR(40) NOT NULL DEFAULT 'house'",
        'possession_status' => "VARCHAR(40) NOT NULL DEFAULT 'ready'",
    ];

    foreach ($cols as $col => $def) {
        $row = $db->query("SHOW COLUMNS FROM `properties` LIKE '$col'")->fetch();
        if (!$row) {
            $log[] = ['error', "Column `$col` not found on `properties` — schema drift, aborting."];
            continue;
        }
        $type = strtolower((string)$row['Type']);
        if (strpos($type, 'varchar') === 0) {
            $log[] = ['ok', "Column `$col` is already VARCHAR — no change."];
            continue;
        }
        $db->exec("ALTER TABLE `properties` MODIFY COLUMN `$col` $def");
        $log[] = ['ok', "Converted `$col` from `{$row['Type']}` to VARCHAR(40)."];
    }

    $log[] = ['done', 'Migration 015 complete.'];
} catch (Throwable $e) {
...
<head>
<meta charset="UTF-8">
<title>Migration 015 — properties taxonomy VARCHAR</title>
...
<body>
  <div class="card p-4">
    <h3 class="mb-3">Migration 015 — <code>properties</code> taxonomy → VARCHAR</h3>
    ...
    <p class="mt-3 mb-0 fs-13 text-muted">
      Next: open <code>/admin/settings.php?tab=listings</code> to add custom Listing Types and Possession Statuses.
    </p>
  </div>
</body>
</html>
