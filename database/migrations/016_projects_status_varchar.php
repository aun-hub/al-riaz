<?php
/**
 * Al-Riaz Associates — Migration 016: projects.status → VARCHAR
 *
 * Converts the Project Status ENUM column to VARCHAR(40) so admins can
 * add custom statuses from Settings → Listings without a schema change.
 *
 * Idempotent — checks current column type before altering.
 *
 * Run:
 *   - Browser (localhost): http://localhost/al-riaz/database/migrations/016_projects_status_varchar.php
 *   - SSH (server):        php /path/to/site/database/migrations/016_projects_status_varchar.php
 */
...
try {
    ...
    $row = $db->query("SHOW COLUMNS FROM `projects` LIKE 'status'")->fetch();
    if (!$row) {
        $log[] = ['error', "Column `status` not found on `projects` — schema drift, aborting."];
    } else {
        $type = strtolower((string)$row['Type']);
        if (strpos($type, 'varchar') === 0) {
            $log[] = ['ok', 'Column `status` is already VARCHAR — no change.'];
        } else {
            $db->exec("ALTER TABLE `projects` MODIFY COLUMN `status` VARCHAR(40) NOT NULL DEFAULT 'upcoming'");
            $log[] = ['ok', "Converted `status` from `{$row['Type']}` to VARCHAR(40)."];
        }
    }

    $log[] = ['done', 'Migration 016 complete.'];
} catch (Throwable $e) {
...
<head>
<meta charset="UTF-8">
<title>Migration 016 — projects.status VARCHAR</title>
...
<body>
  <div class="card p-4">
    <h3 class="mb-3">Migration 016 — <code>projects.status</code> → VARCHAR</h3>
    ...
    <p class="mt-3 mb-0 fs-13 text-muted">
      Next: open <code>/admin/settings.php?tab=listings</code> to add or disable Project Statuses.
    </p>
  </div>
</body>
</html>
