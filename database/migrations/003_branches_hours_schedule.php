<?php
/**
 * Al-Riaz Associates — Migration 003: Add `hours_schedule` JSON column to `branches`
 *
 * Run once from localhost:
 *   http://localhost/al-riaz/database/migrations/003_branches_hours_schedule.php
 *
...
try {
    ...
    $exists = (bool)$db->query("SHOW COLUMNS FROM `branches` LIKE 'hours_schedule'")->fetch();
    if ($exists) {
        $log[] = ['ok', 'Column `hours_schedule` already present — nothing to do.'];
    } else {
        $db->exec("ALTER TABLE `branches` ADD COLUMN `hours_schedule` LONGTEXT DEFAULT NULL AFTER `hours`");
        $log[] = ['ok', 'Added `hours_schedule` column to `branches`.'];
    }

    $log[] = ['done', 'Migration 003 complete.'];
...
<head>
<meta charset="UTF-8">
<title>Migration 003 — branches.hours_schedule</title>
...
<body>
  <div class="card p-4">
    <h3 class="mb-3">Migration 003 — <code>branches.hours_schedule</code></h3>
